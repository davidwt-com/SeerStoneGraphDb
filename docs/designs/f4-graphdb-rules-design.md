...
- Non-instantiable class marker (D15): the abstract `Rule` root is
  seeded with `instantiable => false`, enforced by `create_instance`
  and suppressing the auto-default template. (Deferred as standalone
  work but carried here because Phase A seeds an abstract meta-class
  under nref 3 — see the open packaging note in D15.)
...
| 9   | `Rule` class                          | `graphdb_class:create_class("Rule", ?NREF_CLASSES)` seeded **non-instantiable** (D15 — `instantiable => false`; skips auto-default template) | nref 3 (Classes); abstract root                                         |
...
- The `graphdb_attr:create_relationship_attribute_pair/3` API (the
  no-parent arity) parks both arc-label nodes directly under nref 8
  (`?NREF_RELATIONSHIPS`), with the `TargetKind = instance` argument
  stamped only as a routing AVP (`target_kind => instance`). This is
  the L7-shipped default. (L8 added the `/4` arity with an explicit
  `ParentNref`, so any attribute parent — e.g. nref 16 — is now
  reachable without further API work.)
...
- **Add a kind-specific-parent variant** of
  `create_relationship_attribute_pair` so arc-label nodes can be parked
  under nref 13/14/15/16. *(This is the exit L8 took: the `/4` arity now
  accepts any attribute parent.)*
...
| D15 | Non-instantiable (abstract) class marker — `instantiable => false` AVP on the class node, enforced by `create_instance`; an abstract class skips the auto-default template; abstractness is set at creation. Deferred as standalone work, **but required wherever meta-classes are seeded under nref 3 (Classes)**. F4 Phase A seeds the abstract `Rule` root, so it carries this marker; `CompositionRule` / `ConnectionRule` stay instantiable. | 2026-06-01 |
