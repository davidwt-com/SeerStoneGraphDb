...
%% Description: nref_sup is the top level supervisor for the nref servers.
%%  
...
%% Revision History
%%--------------------------------------------------------------------- 
%% Rev PA1 Date: August 24, 2008 Author: Dallas Noyes (dallas.noyes@gmail.com)
...
%%---------------------------------------------------------------------
-revision('Revision: 1 ').
-created('Date: October 8, 2008 15:08:00').
...
start_link() ->
	supervisor:start_link({local, nref_sup}, nref_sup, []).
...
%%    * one_for_all - if one child process terminates and should be restarted, all other
...
%%      
%%  				The functions terminate_child/2, delete_child/2 and restart_child/2 are invalid for 
%% 					simple_one_for_one supervisors and will return {error,simple_one_for_one} .
%%
...
%% This is a template for a supervisor
	%% Define SUP1 child_spec
		Id = Sup_Name, 			%%Id = term() is the name of the child, often the same as the module name (as here)
