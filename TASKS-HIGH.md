...
H4 and H5 are isolated bugs in `graphdb_instance` and can be fixed before
the multi-inheritance work (H1–H3). H1–H3 require API/schema-shape
changes for multi-parent / multi-class semantics and should land together.

---

## H4. `resolve_from_class` does not walk the class taxonomy
...
## H5. Priority 4 ("directly connected nodes") double-walks Priorities 2 and 3
...
**Fix (after C2):** filter `Rels` to `R#relationship.kind =:= connection`
before pulling target nrefs.

**Fix (before C2, interim):** filter out characterizations equal to
`?CLASS_MEMBERSHIP_ARC`, `?INST_PARENT_ARC`, `?INST_CHILD_ARC`. Ugly but
correct.

**Dependencies:** clean fix depends on C2.

---

## H1. Classes support only single inheritance
...
**Dependencies:** clean implementation benefits from C2 (filter arcs by
kind=taxonomy).

---

## H2. Instances support only single class membership
...
**Dependencies:** none structurally; the resolver work is H3.

---

## H3. `resolve_from_class` silently picks the first class membership
...
ancestors per H4), look up the AVP. If multiple distinct values are
...
**Dependencies:** H2 (so the multi-membership case is reachable), H4
